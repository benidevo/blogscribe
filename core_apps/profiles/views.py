...
from blogscribe.settings.local import DEFAULT_FROM_EMAIL
    ...
    def get(self, request, format=None):
        try:
            ...
            formatted_response = {
                ...
                "followers_profiles": follower_profiles.count(),
...
class FollowingListView(APIView):
    def get(self, request, user_id, format=None):
        try:
            ...
            formatted_response = {
                "status_code": status.HTTP_200_OK,
                "following_profiles": following_profiles.count(),
...
class FollowAPIView(APIView):
    def post(self, request, user_id, format=None):
        try:
            ...
            message = f"Hi there, {profile.user.first_name}!! the user {user_profile.user.first_name} {user_profile.user.first_name} now follows you"  # noqa
            ...
            return Response(
                {
                    "status_code": status.HTTP_200_OK,
                    "message": f"You are not following {user_profile.user.first_name} {user_profile.user.first_name}",  # noqa
                }
            )
        except Profile.DoesNotExist:
...
class UnfollowAPIView(APIView):
    def post(self, request, user_id, *args, **kwargs):
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                "status_code": status.HTTP_400_BAD_REQUEST,
                "message": f"You cannot unfollow {profile.user.first_name} {profile.user.first_name} since you are not following them in the first place",  # noqa
            }
            return Response(formatted_response, status=status.HTTP_400_BAD_REQUEST)
